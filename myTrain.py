...
if __name__ == '__main__':
    ...
    ##################model_name#############################
    model_name = 'DepthFusionPolypPVT'
    ###############################################
...
# 
    pvt_encoder_params = model.polyp_pvt.backbone.parameters()
    depth_branch_params = model.depth_branch.parameters()

    other_polyppvt_params = [p for n, p in model.polyp_pvt.named_parameters() if 'backbone' not in n]

    params_list = [
            {'params': pvt_encoder_params, 'lr': opt.lr * 0.01},
            
            {'params': depth_branch_params, 'lr': opt.lr},
    ...
    if opt.optimizer == 'AdamW':
        ...
        optimizer = torch.optim.SGD(params_list, weight_decay=1e-4, momentum=0.9)
