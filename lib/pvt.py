...
class DepthBranch(nn.Module):
    def __init__(self, channel=32):
        ...
        # Define depth branch layers here
        self.conv1block = nn.Sequential(
            nn.Conv2d(3, 6, kernel_size=2, stride=2),
            nn.ReLU(),
...
class DepthFusePolypPVT(nn.Module):
    ...
    def forward(self, x, depth):
        ...
        # CIM
        x1 = self.polyp_pvt.ca(x1) * x1 # channel attention
        cim_feature = self.polyp_pvt.sa(x1) * x1 # spatial attention


        # transform channel from encoder branch
        x2_t = self.polyp_pvt.Translayer2_1(x2)  
        x3_t = self.polyp_pvt.Translayer3_1(x3)  
        ...
        # SAM
        T2 = self.polyp_pvt.Translayer2_0(cim_feature)
        T2 = self.polyp_pvt.down05(T2)
        sam_feature = self.polyp_pvt.SAM(cfm_feature, T2)

        prediction1 = self.polyp_pvt.out_CFM(cfm_feature)
        prediction2 = self.polyp_pvt.out_SAM(sam_feature)
